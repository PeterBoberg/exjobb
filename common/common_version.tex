...
\begin{document}
	...
	Chapter \ref{section:methods} describes the methods and methodology used in the project. This includes the research strategy, describing how the application is developed and how it is evaluated.
	...
	Chapter \ref{section:discussion} discusses the result and methods used in the study.
	...
	The result shows that the number of \textit{false negatives} were 9/25 on the falls scenario, and the number of \textit{false positives} were 7/25 on the jumping scenario. This means that their algorithm missed 9 out of 25 actual falls, and falsely flagged 7 out of 25 jumps as falls. In all other scenarios the number of \textit{false negatives/false positives} was zero. They conclude by saying that their algorithm had a hard time distinguishing between falling and jumping, but performed good in the other scenarios. 
	...
	Abbate et al. tries to establish their algorithm using a \textit{finite-state-machine} with five distinct states, seen in figure \ref{fig:abbatea-state-machine}.
	...
	An \textit{inductive} research method \cite{inductive} can be viewed as a bottom-up approach, where the researcher begins by collecting data that is relevant to the subject. After that, the data is analyzed, with the intention to develop a theory that explains the patterns observed in the data.
	
	A \textit{deductive} method \cite{deductive} is a top-down approach, where a hypothesis is developed from existing theory. After the hypothesis is formulated, it is tested with the intent to confirm or falsify it.
	...
	\begin{figure}[H]
		\centering
		\subfloat{{\includegraphics[width=9cm]{../img/architecture-outline.png} }}%
	...
	The state machine uses a number of thresholds. They can be seen in figure \ref{fig:thresholds} and are defined as:
	...
	\begin{figure}[H]
		...
		\subfloat{{\includegraphics[width=14cm]{../img/fall-data-run-and-fall-ios.png} }}%
	...
	\begin{figure}[H]
		...
		\subfloat{{\includegraphics[width=14cm]{../img/fall-data-run-and-fall-android.png} }}%
