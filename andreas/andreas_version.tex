...
\begin{document}
	...
	\begin{figure}[H]
		\centering
		\subfloat{{\includegraphics[width=10cm]{../img/state-machine.png} }}%
	...
	The purpose of the feature extractor is to take the data in the ring buffer and extract 8 defined features from it. The 8 features will then be used in the neural network. The features are similar to the features defined by Abbatea et al. \cite{piza_uni}, but redefined.
	
	\begin{figure}[H]
		\centering
		\subfloat{{\includegraphics[width=14cm]{../img/fall-data-example.png} }}%
		\caption{Recorded data from dropping the phone. Each point on the graph represents one sample in the ring buffer.}%
